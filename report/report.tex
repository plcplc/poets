...
\usepackage[colorlinks, filecolor=blue, urlcolor=blue]{hyperref}
...
\begin{figure}
  \begin{equation*}
    \frac{
      \Gamma |- C_1 => e_1 : ~\alpha
      \quad \Gamma |- C_2 => e_2 : \beta
      \quad \Gamma |- C_3 => e_3 : \gamma
    }{
      \Gamma |- C[Bool/\alpha][\gamma/\beta] ~\kw{if}~ e_1 ~\kw{then}~ e_2 ~\kw{else}~ e_3 : \gamma
...
\begin{equation*}
  \frac{
    \Gamma\{id : \alpha\} |- C => e : \beta
  }{
    \Gamma |- C => \lambda id -> e : \alpha -> \beta
  }
...
\begin{equation*}
  \frac{
    ...
    \Gamma |- C[(\beta -> \gamma)/\alpha] => e_1~ e_2 : \gamma
...
\fxnote{example}. Alternative, we can use a binding environment
...
% which is passed along where appropriate. The need for an environment
% is facilitated by the lambda abstraction; this also means the
% semantics of the expression evaluation is extended with this
% environment, while only utilized by the lambda evaluation algebra
% instance. 
We have selected to use the environment approach, as we find this most
modular. \fxwarning{mere argumentation} These environment based
