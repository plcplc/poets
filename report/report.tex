...
also contains information about when this should happen and a
predicate \fxwarning{explain usage scenario}. This predicate language
was very limited in the Tom Hvitveds incarnation, for instance, while
CSL supports lists, the predicate language had no primitives for
manipulating them and provides no means of defining important
...
the existing system. \fxnote{applications burde måske erstattes med noget andet}
...
\paragraph{Case 2: Abstractions} The current language doesn't provide
...
& \quad\quad || \kw{fun}~  id : type~ (id_1 : type_1, id_2 : type_2, \ldots) = e \\
  e ::= &\ \ldots || \lambda id -> e || e_1\ e_2 || \kw{if}~ e~ \kw{then}~ c_1~ \kw{else}~ c_2 \tag{CSL predicates} \\
  type ::= &\ \kw{Int} || \kw{Bool} || \ldots || type_1 -> type_2 || typevar \tag{CSL types}
\end{align*}
